...
//            console.log(arg, );
        ...
        function goto_item(arg, e,pargs){
            ...
            // console.info('goto_item', arg,e,pargs);
            // -- arg = object
            var _t = arg;
            var args = {};
            var arg = '';
            ...
            // console.warn("o.preset_name -> ",o.preset_name);
            // console.warn("_t.attr -> ",_t.attr, _t.attr);
            // console.warn("_t.attr('data-preset-name') -> ",_t.attr('data-preset-name') );



            if(_t){
                if(_t.attr && _t.attr('data-preset-name')){
                    // console.info("_t.attr('data-preset-name') -> ",_t.attr('data-preset-name'));
                    if(_t.attr('data-preset-name') != o.preset_name){
                        // console.info("_t.attr('data-preset-name') DIFFERENT -> ",_t.attr('data-preset-name'));


                        destroy();

                        if(_t.attr('data-preset-name')=='whitefull'){
                            window.init_zoombox_settings = window.init_zoombox_whitefull;
                        }

                        if(_t.attr('data-preset-name')=='darkfull'){
                            window.init_zoombox_settings = window.init_zoombox_darkfull;
                        }
...
//                console.info($.fn.vPlayer)
            ...
            // console.info('title - ', margs.title);
...
//                    console.log(_cach, _t, _cach==_t);
        ...
        function gotoItem(arg, argtype, otherargs) {
            ...
            if(o.settings_deeplinking=='on' && has_history_api()==true && otherargs.forcenodeeplink!='on'){
                ...
                theurl = newurl;
                // console.info('PUSING STATE -3', theurl);
                history.pushState({}, "", newurl);
            }
            ...
            var holder_html = otherargs.item.find('.zoombox-larger-description').eq(0).html();
            var holder_text = otherargs.item.find('.zoombox-larger-description').eq(0).text();

            // console.info('holder_html - ',holder_html);
            // console.info('holder_text - ',holder_text);
            ...
            if(o.design_skin=='skin-darkfull'){
                if(otherargs.item.find('.zoombox-larger-description').length>0){
                    aux+=holder_html;
            ...
            if(holder_text){
                ...
                _zoombox_maincon.find('.info-btn').removeClass('disabled');
            }else{

                // console.info("NOTNOT");
                _zoombox_maincon.find('.info-btn').addClass('disabled');
            }
                    ...
                    if(_c.children().length==0){
                        ...
                        if(_theItem.find('.slider-con').length>0){
                            ...
                            if(_c.children('.advancedscroller').length>0){
                                _c.children('.arrow-left-for-skin-qucreative').remove();
                                _c.children('.arrow-right-for-skin-qucreative').remove();

                                _c.children('.advancedscroller').removeClass('skin-nonav').addClass('skin-qucreative');
                ...
                _theItem.find('.toexecute').each(function(){
                    ...
                    try{
                        ...
                        if(arr.type=='transform_slider_con'){
                            window.dzsas_init(window.dzszb_execute_target.find(".slider-con .advancedscroller"),arr);
                        }

                        if(arr.type=='item_excerpt_setup'){
                            window.dzsas_init(window.dzszb_execute_target.find(".advancedscroller"),arr);
                        }
...
//                    console.info(bigwidth, bigheight);
                        ...
                        if(typeof otherargs.item !='undefined'){
                            ...
                            // console.info(otherargs.item.find('.cover-image'))
                            if(otherargs.item.find('.cover-image').length){
                                if(otherargs.item.find('.cover-image').length>0){
                                    aux+=''+otherargs.item.find('.cover-image').eq(0).outerHTML()+'';
                                }
                            }
                        }
...
//            console.info(otherargs);
            ...
            // console.log("arg, TYPE, _theItem" , arg, argtype, _theItem);


            //console.log(has_history_api());

            // console.info('func_callback -> ', func_callback);
            if(func_callback){
...
//                        console.info(bh, binh, wh, bt);
    ...
    window.init_zoombox = function(args){
        ...
        // console.info('COMING YOUR WAY', $('.zoombox-delegated'), window._zoombox_maincon);



        $(document).off('click.dzszbx','.zoombox-delegated');
        $(document).on('click.dzszbx','.zoombox-delegated',function(){

            // console.warn('open from delegated ;) ');
